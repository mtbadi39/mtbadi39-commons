 ...
 *
 * Copyright (c) 2003-2010 Werner Randelshofer Hausmatt 10, Immensee, CH-6405,
 * Switzerland.
 *
 * This software is in the public domain. You are free to use, adapt, copy and
 * license this work without having to attribute to Werner Randelshofer.
 */
import java.awt.*;
...
public class SplashWindow extends Window {
     ...
     * called at least once since the construction of this window.<br> This
     * attribute is used to notify method splash(Image) that the window has been
     * drawn at least once by the AWT event dispatcher thread.<br> This
     * attribute acts like a latch. Once set to true, it will never be changed
     * back to false again.
     *
     * @see #paint
     * @see #splash
    ...
    private SplashWindow(JFrame parent, Image image) {
        ...
        if (mt.isErrorID(0)) {
            setSize(0, 0);
            System.err.println("Avertissement : SplashWindow n'a pas pu charger l'image.");
            synchronized (this) {
                paintCalled = true;
                notifyAll();
            }
        ...
        int imgWidth = image.getWidth((Window) this);
        int imgHeight = image.getHeight((Window) this);
     ...
     */
    public static void splash(Image image) throws Exception {
     ...
     * @throws java.lang.Exception
     */
    public static void splash(URL imageURL) throws Exception {
        try {
            if (imageURL != null) {
    ...
    public static void invokeMain(String className, String[] args) throws Exception {
        try {
            ...
            throw new InternalError("SplashWindow : Impossible d'invoquer la méthode principale [Main] du :" + className);
        }
    }
}
